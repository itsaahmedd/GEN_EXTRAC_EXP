...
def retrieve_top_k(query, vectorizer, tfidf_matrix, k=3):
    ...
    # Get indices of the top-k most similar chunks (in descending order)
...
use_chunking = True  # TF-IDF retrieval will be applied when chunking is enabled
...
for model_name, qa_model in models.items():
    ...
    for ctx_item in tqdm(data["contexts"], desc="Contexts"):
        ...
        # Build a TF-IDF index for the chunks
            ...
            for cat_name, questions in category_dict.items():
                ...
                for q_obj in tqdm(questions, desc=f"Questions in {cat_name}", leave=False):
                    ...
                    if use_chunking:
                        ...
                        top_indices, sims = retrieve_top_k(question_text, vectorizer, tfidf_matrix, k=top_k_chunks)
                        # Option 1: Concatenate the top-k chunks into one context string
                        retrieved_context = " ".join([context_chunks[idx] for idx in top_indices])
                        
                        # Run the QA model using the retrieved context
                        result = qa_model({"question": question_text, "context": retrieved_context})
...
with open("detailed_experiment_5_results.json", "w") as outfile:
...
with open("Aggregated_experiment_5_results.json", "w") as outfile:
